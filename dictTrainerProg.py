...
# Working directory to save dictionary words to
while True:
    try: 
        workingDirec = input("Please provide the working directory with your dictionary or where you wish to save your new practice dictionary to! ")
...
#Create pickle file if savedDics.txt does not exist
if "savedDics.txt" not in os.listdir():
    vocab = {}
    vocabFails = {}
    with open('savedDics.txt', 'wb') as file:
        pickle.dump([vocab, vocabFails], file)
    print("Please note that in order to practice vobab, you must first enter words into the dictionary. From the main menu, choose 'Edit vocab' and then 'Add vocab'. Happy learning!")

#mainMenu func for option 1 and option 2
def vocabTest(vocab):
...
def vocabTrainer():
    ...
    while leave != "yes":
        ...
        if menuOpt == 1:
            ...
            if len(vocabFails) == 0:
                print("You have not made any mistakes recently. Please practice or do a test to identify your weaknesses!")
            else:
